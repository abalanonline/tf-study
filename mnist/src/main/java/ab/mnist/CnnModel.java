...
public class CnnModel implements NnModel, Closeable {
  ...
  public CnnModel(String optimizerName) {
    graph = new Graph();
  ...
  public void train(MnistDataset dataset, int batchSize, int maxSize) {
    ...
    for (int i = 0; i < Common.EPOCHS; i++) {
      for (ImageBatch trainingBatch : dataset.trainingBatches(batchSize)) {
  ...
  public void test(MnistDataset dataset, int batchSize) {
    ...
    for (ImageBatch trainingBatch : dataset.testBatches(batchSize)) {
